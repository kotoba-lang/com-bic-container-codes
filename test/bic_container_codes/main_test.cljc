(ns bic-container-codes.main-test
  "Contract + behavioral test for the bic-container-codes L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "bic_container_codes-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'bic-container-codes.main-test)]
