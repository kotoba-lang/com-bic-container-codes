(ns bic-container-codes.main
  "Kotodama WASM entrypoint for the bic_container_codes clean-room actor (L4) — Clojure port.
  ...
  facts (`bic_container_codes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bic_container_codes")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "bicconta_tra"
    :fields [:gtin :description :brand]               :required [:gtin :description]
    :coerce {}                                 :refs {}}
   {:entity "Event"      :plural "events"      :id-prefix "bicconta_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                                 :refs {:itemId "TradeItem"}}
   {:entity "Document"   :plural "documents"   :id-prefix "bicconta_doc"
    :fields [:type :senderId :receiverId :status]    :required [:type :status]
    :coerce {}                                 :refs {}}
   {:entity "Container"  :plural "containers"  :id-prefix "bicconta_con"
    :fields [:bicCode :type :status]                  :required [:bicCode :type]
    :coerce {}                                 :refs {}}
   {:entity "Party"      :plural "parties"     :id-prefix "bicconta_par"
    :fields [:gln :name :role]                        :required [:gln :name]
    :coerce {}                                 :refs {}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "bicconta_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"bic_container_codes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bic_container_codes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bic_container_codes-compat/ready)
